...
Public Sub protInit( _
    ...
    This.cookieHeader = vbNullString
End Sub

'Get whether the session is authenticated.
'@returns - `True` when the session is authenticated. False otherwise.
Public Property Get isAuthenticated() As Boolean
  isAuthenticated = This.isSessionReady
End Property
...
Public Sub protEnsureAuthenticated()
    ...
    Dim cookieHeader As String: cookieHeader = BuildCookieHeader()
    If LenB(cookieHeader) = 0 Then
        This.isSessionReady = False
        Err.Raise 5, "stdSharepointAuthenticator::protEnsureAuthenticated", "No cookies available for URL: " & This.site
    End If
...
'Build a cookie header from current authenticated session.
...
Private Function BuildCookieHeader() As String
    ...
    pairs = This.wv.CookiesForRequest(This.site)
...
Private Function stdICallable_RunEx(ByVal params As Variant) As Variant
    ...
    Dim pHTTP As Object: Set pHTTP = params(0)

    If Not This.isSessionReady Then Call protEnsureAuthenticated
    

    If LenB(This.cookieHeader) = 0 Then Err.Raise 5, "stdSharepointAuthenticator::stdICallable_RunEx", "No cookies available for URL: " & This.site

    Call pHTTP.SetRequestHeader("Cookie", This.cookieHeader)
End Function
